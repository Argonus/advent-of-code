defmodule Day10 do
  ...
  def part_one(file_path) do
    file_path
    |> parse_input()
    |> Stream.map(&remove_closing_chars/1)
    |> Stream.filter(&is_corrupted?/1)
    |> Stream.map(&find_closing/1)
    |> Stream.map(&Map.get(@illegal_chars, &1, 0))
    |> Enum.sum()
  end

  defp is_corrupted?(line) do
    String.contains?(line, @closing_chars)
  end

  defp remove_closing_chars(line) do
    if Enum.any?(@char_pairs, &String.contains?(line, &1)) do
      new_line =
        Enum.reduce(@char_pairs, line, fn pair, line_acc ->
          String.replace(line_acc, pair, "")
        end)

      remove_closing_chars(new_line)
    else
      line
    end
  end

  defp find_closing(chars) do
    chars
    |> String.split("", trim: true)
    |> Enum.find(&Enum.member?(@closing_chars, &1))
  end
  ...
  def part_two(file_path) do
    ...
    |> parse_input()
    |> Stream.map(&remove_closing_chars/1)
    |> Stream.reject(&is_corrupted?/1)
    |> Stream.map(&completion_score/1)
    |> Enum.to_list()
    |> median()
  end

  @completion_chars_scores %{
    ")" => 1,
    "]" => 2,
    "}" => 3,
    ">" => 3
  ...
  defp completion_score(chars) do
    ...
    |> Enum.reduce(0, fn char, acc ->
      acc * 5 + Map.get(@completion_chars_scores, char)
    end)
  end
