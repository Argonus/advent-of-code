defmodule Day06 do
  ...
  lanternfish - one lanternfish might have 2 days left until it creates another lanternfish,
  while another might have 4. So, you can model each fish as a single number that represents
  ...
  def part_one(file_path) do
    ...
    |> parse_file()
    |> init_population()
    |> calculate_population(@part_one_days)
    |> sum_population()
  end

  defp init_population(population) do
    Enum.reduce(population, @fish_population, fn fish, acc ->
      Map.update!(acc, fish, &(&1 + 1))
    end)
  end

  defp calculate_population(fish_population, 0), do: fish_population

  defp calculate_population(fish_population, days_left) do
    new_population = Enum.reduce(fish_population, @fish_population, &fish_breed/2)
    calculate_population(new_population, days_left - 1)
  end

  defp fish_breed({0, population}, acc) do
    acc
    |> Map.update!(6, &(&1 + population))
    |> Map.replace!(8, population)
  end

  defp fish_breed({day, population}, acc) do
    Map.update!(acc, day - 1, &(&1 + population))
  end

  defp sum_population(population) do
  ...
  def part_two(file_path) do
    ...
    |> parse_file()
    |> init_population()
    |> calculate_population(@part_two_days)
    |> sum_population()
  end
end
